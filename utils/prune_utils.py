...
weight_data_file='weight_data.txt'
...
class BNOptimizer():
    ...
    def updateBN_Sparse_SA(sr_flag, module_list, s, prune_idx, epoch, idx2mask=None, opt=None):
        if sr_flag:
            # s = s if epoch <= opt.epochs * 0.5 else s * 0.01
            #s=optimizer2.param_groups[-1]['params']
            for idx in prune_idx:
                # Squential(Conv, BN, Lrelu)
                s_tensor=torch.tensor(s[idx])
                bn_module = module_list[idx][1]
                bn_module.weight.grad.data.add_(s_tensor * torch.sign(bn_module.weight.data))  # L1
                # with open(weight_data_file,'a') as f4:
                #     f4.write(torch.sign(bn_module.weight.data))
                #bn_module.weight.grad.data.add_(s * (0.5 * torch.sign(bn_module.weight.data)/(bn_module.weight.data.abs()**0.5))) # L1/2
            if idx2mask:
                for idx in idx2mask:
                    bn_module = module_list[idx][1]
                    #bn_module.weight.grad.data.add_(0.5 * s * torch.sign(bn_module.weight.data) * (1 - idx2mask[idx].cuda()))
                    #bn_module.weight.grad.data.sub_(0.99 * s * (0.5 * torch.sign(bn_module.weight.data)/(bn_module.weight.data.abs()**0.5))*idx2mask[idx].cuda()) #L1/2
                    bn_module.weight.grad.data.sub_(0.99 * s * torch.sign(bn_module.weight.data) * idx2mask[idx].cuda()) # L1
    #两种方法结合
    def updateBN_Sparse_tlp_SA(pinTLp,sr_flag, module_list, s, prune_idx, epoch, idx2mask=None, opt=None):
        p=pinTLp
        if sr_flag:
            for idx in prune_idx:
                s_tensor=torch.tensor(s[idx])
                bn_module = module_list[idx][1]
                bn_module.weight.grad.data.add_(s_tensor * torch.sign(bn_module.weight.data)*p**2*(bn_module.weight.data.abs()**(p-1))/(p+(1-p)*bn_module.weight.data.abs())**2)  # L1
                if idx2mask:
                    for idx in idx2mask:
                        bn_module = module_list[idx][1]
                        bn_module.weight.grad.data.sub_(0.99 * s_tensor * torch.sign(bn_module.weight.data)*p**2*(bn_module.weight.data.abs()**(p-1))/(p+(1-p)*bn_module.weight.data.abs())**2 * idx2mask[idx].cuda()) # L1
    
    def updateBN_Lp(sr_flag, module_list, s, prune_idx, epoch, idx2mask=None, opt=None):
        if sr_flag:
            # s = s if epoch <= opt.epochs * 0.5 else s * 0.01
            p = 0.5
            for idx in prune_idx:
                ...
                bn_module = module_list[idx][1]
                #bn_module.weight.grad.data.add_(s * torch.sign(bn_module.weight.data))  # L1
                bn_module.weight.grad.data.add_(s * (p * torch.sign(bn_module.weight.data)/(bn_module.weight.data.abs()**(1-p)))) # L1/2
            if idx2mask:
                for idx in idx2mask:
                    bn_module = module_list[idx][1]
                    #bn_module.weight.grad.data.add_(0.5 * s * torch.sign(bn_module.weight.data) * (1 - idx2mask[idx].cuda()))
                    bn_module.weight.grad.data.sub_(0.99 * s * (p * torch.sign(bn_module.weight.data)/(bn_module.weight.data.abs()**(1-p)))*idx2mask[idx].cuda()) #L1/2
                    #bn_module.weight.grad.data.sub_(0.99 * s * torch.sign(bn_module.weight.data) * idx2mask[idx].cuda()) # L1

    def updateBN_TLp(sr_flag, module_list, s, prune_idx, epoch, idx2mask=None, opt=None):
        p = 0.5
        if sr_flag:
            for idx in prune_idx:
                bn_module = module_list[idx][1]
                bn_module.weight.grad.data.add_(s*torch.sign(bn_module.weight.data)*p**2*(bn_module.weight.data.abs()**(p-1))/(p+(1-p)*bn_module.weight.data.abs())**2) # TLp
            if idx2mask:
                for idx in idx2mask:
                    bn_module = module_list[idx][1]
                    bn_module.weight.grad.data.sub_(0.99 * s*torch.sign(bn_module.weight.data)*p**2*(bn_module.weight.data.abs()**(p-1))/(p+(1-p)*bn_module.weight.data.abs())**2*idx2mask[idx].cuda()) #TLp

    def updateBN_TLp_dynamic(num_no_increase_epoch, sr_flag, module_list, s, prune_idx, epoch, idx2mask=None, opt=None):
        #p = (opt.epochs - 0.5 * epoch) / opt.epochs
        p = 1-0.5*(epoch-num_no_increase_epoch)/opt.epochs
        if sr_flag:
            for idx in prune_idx:
                bn_module = module_list[idx][1]
                bn_module.weight.grad.data.add_(s*torch.sign(bn_module.weight.data)*p**2*(bn_module.weight.data.abs()**(p-1))/(p+(1-p)*bn_module.weight.data.abs())**2) # TLp
    ...
    def updateBN_TLp_SA(pinTLp, sr_flag, module_list, s, prune_idx, epoch, idx2mask=None, opt=None):
        #p = (opt.epochs - 0.5 * epoch) / opt.epochs
        #p = 1-0.5*(epoch-num_no_increase_epoch)/opt.epochs
        p=pinTLp
        if sr_flag:
    ...
    def updateBN_TLp_SA_layerwise(pinTLp_list, idx2cnt, sr_flag, module_list, s, prune_idx, epoch, idx2mask=None, opt=None):
        #p = (opt.epochs - 0.5 * epoch) / opt.epochs
        #p = 1-0.5*(epoch-num_no_increase_epoch)/opt.epochs
        p_list=pinTLp_list
        if sr_flag:
            for idx in prune_idx:
                p = p_list[idx2cnt[idx]]
                bn_module = module_list[idx][1]
                bn_module.weight.grad.data.add_(s*torch.sign(bn_module.weight.data)*p**2*(bn_module.weight.data.abs()**(p-1))/(p+(1-p)*bn_module.weight.data.abs())**2) # TLp
            if idx2mask:
                for idx in idx2mask:
    ...
    def updateBN_TL1(sr_flag, module_list, s, prune_idx, epoch, idx2mask=None, opt=None):
        a = 0.5
        if sr_flag:
            for idx in prune_idx:
                bn_module = module_list[idx][1]
                bn_module.weight.grad.data.add_(s*a*(a+1)*torch.sign(bn_module.weight.data)/(a+bn_module.weight.data.abs())**2) # TL1
            if idx2mask:
                for idx in idx2mask:
                    bn_module = module_list[idx][1]
                    bn_module.weight.grad.data.sub_(0.99 * s*a*(a+1)*torch.sign(bn_module.weight.data)/(a+bn_module.weight.data.abs())**2*idx2mask[idx].cuda()) #TL1
