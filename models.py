...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, img_size, var=None):
        ...
        if self.training:
            ...
            io = p.new_tensor(p)  # inference output
            ...
            io_view = io.view(bs, -1, 5 + self.nc)
...
def create_grids(self, img_size=416, ng=(13, 13), device='cpu', type=torch.float32):
    ...
    yv_item = torch.arange(ny)
    yv_item = yv_item.unsqueeze(1)
    yv = torch.cat([yv_item for i in range(1,nx+1)], 1)
    xv_item = torch.arange(nx)
    xv_item = xv_item.unsqueeze(0)
    xv = torch.cat([xv_item for i in range(1,ny+1)], 0)
